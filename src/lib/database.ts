...
export type DocumentType = 'supplier' | 'invoice' | 'company' | 'cost_center' | 'payment_terms' | 'event' | 'lob' | 'fund';
...
export const CREATE_DOCUMENTS_TABLE = `
  CREATE TABLE IF NOT EXISTS documents (
    ...
    type VARCHAR(20) NOT NULL CHECK (type IN ('supplier', 'invoice', 'company', 'cost_center', 'payment_terms', 'event', 'lob', 'fund')),
...
export const MIGRATIONS = [
  ...
  `ALTER TABLE documents ADD CONSTRAINT documents_type_check CHECK (type IN ('supplier', 'invoice', 'company', 'cost_center', 'payment_terms', 'event', 'lob', 'fund'))`,
];
