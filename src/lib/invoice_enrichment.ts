...
export async function enrichInvoiceFromAttachments(
  ...
  try {
    ...
    const companySearchInstructions = ' When calling findCompanies, pass the billed company name or Company_Reference_ID in query and the bill-to street address in address. Never concatenate street, city, state, or ZIP into query. If addressMatch is unique, treat that as a strong extra signal. If it is shared, ignore address when choosing among those companies.';
