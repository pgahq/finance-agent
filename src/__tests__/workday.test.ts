...
describe('Workday utilities', () => {
  ...
  describe('submitSupplierInvoiceUpdate', () => {
    ...
    it('should rethrow validation faults after a fallback retry fails', async () => {
      ...
      await expect(
        ...
        message: 'Validation_Fault: invoice date must be valid',
        priorFailures: [
          { attempt: 1, message: 'Validation_Fault: invoice date must be valid' },
          { attempt: 2, fallback: 'default invoice date', message: 'Validation_Fault: invoice date must be valid' },
        ],
      });
    ...
    it('should not retry validation faults without a matching configured fallback field', async () => {
      ...
      const rejected = submitSupplierInvoiceUpdateForTest({
        invoiceDate: '2025-04-15'
      });
      await expect(rejected).rejects.toMatchObject({
        message: 'Validation_Fault: duplicate payload should not be retried',
      });
    ...
    it('should prefer a provided memo over an existing invoice memo', async () => {
      const mockClient = {
        setSecurity: jest.fn(),
        setEndpoint: jest.fn(),
        Get_Supplier_Invoices: jest.fn(),
    ...
    it('should keep the existing memo when no replacement memo is provided', async () => {
      const mockClient = {
        setSecurity: jest.fn(),
        setEndpoint: jest.fn(),
        Get_Supplier_Invoices: jest.fn(),
      ...
      const existingMemo = 'Existing memo from OCR';
      const mockGetResponse = {
        Response_Data: {
          Supplier_Invoice: {
            Supplier_Invoice_Data: {
    ...
    describe('finalLines', () => {
      ...
      describe('line Tax_Applicability_Reference', () => {
        ...
        it('should set USA Taxable when header tax comes from the existing Workday Tax_Amount', async () => {
          const { mockClient, getCapturedRequest } = setupMockClient();
          mockClient.Get_Supplier_Invoices.mockImplementation((_request: any, callback: any) => {
            callback(null, {
              Response_Data: {
        ...
        it('should retry without Tax_Applicability_Reference when Workday rejects line tax applicability', async () => {
          ...
          mockClient.Submit_Supplier_Invoice.mockImplementation((request: any, callback: any) => {
            ...
            if (capturedRequests.length === 1) {
              callback({
                Validation_Fault: {
                  Validation_Error: {
                    Message: 'Tax Code is required when Tax Applicability is entered.',
                    Xpath: '/wd:Submit_Supplier_Invoice_Request[1]/wd:Supplier_Invoice_Data[1]/wd:Invoice_Line_Replacement_Data[1]/wd:Tax_Applicability_Reference'
                  }
                }
              }, null);
              return;
          ...
          const firstData = capturedRequests[0].Submit_Supplier_Invoice_Request.Supplier_Invoice_Data;
          const retryData = capturedRequests[1].Submit_Supplier_Invoice_Request.Supplier_Invoice_Data;
          expect(firstData.Invoice_Line_Replacement_Data[0].Tax_Applicability_Reference).toEqual(usaTaxableRef);
          expect(retryData.Invoice_Line_Replacement_Data[0].Tax_Applicability_Reference).toBeUndefined();
          expect(retryData.Tax_Amount).toBe(45);
          expect(result.appliedFallbacks).toEqual(
      ...
      it('should replace cost center with fallback on a cost-center availability fault', async () => {
        const mockClient = {
          setSecurity: jest.fn(),
          setEndpoint: jest.fn(),
          Get_Supplier_Invoices: jest.fn(),
        ...
        mockClient.Submit_Supplier_Invoice.mockImplementation((request: any, callback: any) => {
          ...
          if (capturedRequests.length === 1) {
            callback({
              Validation_Fault: {
                Validation_Error: {
                  Message: 'The Cost Center is/are not available for use with the company/s: CC-Technology Services',
      ...
      it('should apply a related LOB instead of fallback cost center when Workday also says the cost center is unavailable', async () => {
          ...
          callback(null, {
            ...
            Response_Data: {
              Related_Worktags: {
                Related_Worktag_Reference: {
                  ID: [
                    { $attributes: { type: 'WID' }, $value: 'cc-ent-tech-wid' },
                    { $attributes: { type: 'Cost_Center_Reference_ID' }, $value: 'CC-Enterprise Technology' }
                  ]
                },
                Related_Worktags_Data: {
                  Related_Worktags_by_Type_Data: {
                    Worktag_Type_Reference: {
                      ID: [{ $attributes: { type: 'Worktag_Type_ID' }, $value: 'LINE_OF_BUSINESS' }]
                    },
                    Required_On_Transaction: true,
                    Allowed_Worktag_Data: [
                      { Allowed_Worktag_Reference: { ID: [{ $attributes: { type: 'Organization_Reference_ID' }, $value: 'LOB-Enterprise' }] } }
        ...
        rmClient.Submit_Supplier_Invoice.mockImplementation((request: any, callback: any) => {
          capturedRequests.push(request);
          const worktags = JSON.stringify(request?.Submit_Supplier_Invoice_Request?.Supplier_Invoice_Data?.Invoice_Line_Replacement_Data?.[0]?.Worktags_Reference ?? []);
          if (!worktags.includes('LOB-Enterprise')) {
            callback({
              ...
              detail: {
                Validation_Fault: {
                  Validation_Error: [
                    {
                      Message: 'When "Cost Center: CC-Enterprise Technology" is entered then these worktag types must also have a value: Line of Business.',
                      Detail_Message: 'Parm Supplier_Invoice_Line_Replacement_Data (6$18900): n, Worktags_for_Procurement_Webservices--IS Restricted by Supplier Invoice Line Replacement Data',
        ...
        const result = await submitSupplierInvoiceUpdateForTest({
          finalLines: [
            { lineOrder: 1, description: 'Service', quantity: 1, unitCost: 100, extendedAmount: 100, fundId: 'FUND-General_Fund_Unrestricted', costCenterId: 'CC-Enterprise Technology' },
            { lineOrder: 9, description: 'Line 9', quantity: 1, unitCost: 100, extendedAmount: 100, fundId: 'FUND-General_Fund_Unrestricted', costCenterId: 'CC-Enterprise Technology' }
          ],
          relatedLobByCostCenter: new Map([
            ['CC-Enterprise Technology', EMPTY_RELATED_LOB]
          ]),
          resolveCostCenterWorkdayIds: async () => new Map([
            ['CC-Enterprise Technology', 'cc-ent-tech-wid']
          ])
        });
        delete process.env.FALLBACK_LOB_ID;
        delete process.env.FALLBACK_COST_CENTER_ID;
        ...
        expect(fmClient.Get_Related_Worktags_for_Worktags).toHaveBeenCalledWith(
          expect.objectContaining({
            Get_Related_Worktags_for_Worktags_Request: expect.objectContaining({
              Request_References: {
                Related_Worktag_Reference: expect.arrayContaining([
                  { ID: [{ $attributes: { type: 'Cost_Center_Reference_ID' }, $value: 'CC-Enterprise Technology' }] },
                  { ID: [{ $attributes: { type: 'WID' }, $value: 'cc-ent-tech-wid' }] }
                ])
              }
            })
          }),
        ...
        const retryLines = capturedRequests[1].Submit_Supplier_Invoice_Request.Supplier_Invoice_Data.Invoice_Line_Replacement_Data;
        expect(retryLines).toHaveLength(2);
        for (const line of retryLines) {
          expect(line.Worktags_Reference).toEqual([
            { ID: [{ $attributes: { type: 'Fund_ID' }, $value: 'FUND-General_Fund_Unrestricted' }] },
            { ID: [{ $attributes: { type: 'Cost_Center_Reference_ID' }, $value: 'CC-Enterprise Technology' }] },
            { ID: [{ $attributes: { type: 'Organization_Reference_ID' }, $value: 'LOB-Enterprise' }] }
          ]);
  ...
  describe('submitNewSupplierInvoice', () => {
    ...
    it('keeps org venue passthrough alongside LOB and on split parents', async () => {
      const mockClient = mockSoapClient();
      ...
      const makeOrgWorktag = (value: string) => ({
        ID: [
          { $attributes: { type: 'WID' }, $value: `wid-${value}` },
      ...
      await submitNewSupplierInvoiceForTest({
        finalLines: [
          {
            lineOrder: 1,
            description: 'Non-split line with LOB',
            quantity: 1,
            unitCost: 100,
            extendedAmount: 100,
            fundId: 'FUND-INV',
          ...
          {
            lineOrder: 2,
            description: 'Split line with venue',
            quantity: 1,
            unitCost: 100,
            extendedAmount: 100,
            fundId: 'FUND-INV',
      ...
      const nonSplitValues = valuesFor(lines[0]);
      expect(nonSplitValues).toContain('Organization_Reference_ID:VENU-Contestant_Indirect');
      expect(nonSplitValues.filter((v: string) => v.endsWith(':LOB-Technology_Services'))).toHaveLength(1);

      const splitValues = valuesFor(lines[1]);
    ...
    describe('line Tax_Applicability_Reference', () => {
      ...
      it.each([
        ...
        ['$0.00', false],
        ['not a number', false],
      ])('with extractedTaxAmount %p tags lines: %p', async (extractedTaxAmount, tagged) => {
        const mockClient = mockSoapClient();
        ...
        await submitNewSupplierInvoiceForTest({ extractedTaxAmount });

        const line = capturedRequest.Submit_Supplier_Invoice_Request.Supplier_Invoice_Data.Invoice_Line_Replacement_Data[0];
        expect(line.Tax_Applicability_Reference).toEqual(tagged ? usaTaxableRef : undefined);
      });
