...
- **findCompanies**: Search our company database using semantic similarity to find relevant companies (the buyer/recipient entity on the invoice). Call it with the billed company name or Company_Reference_ID in \`query\` and the bill-to street address in \`address\`. Never concatenate street, city, state, or ZIP into \`query\`. Prefer any company tagged unique or shared over a name-only hit that did not match the bill-to street. Unique is a strong extra signal. Shared means several returned companies sit on that street: ignore address only among those shared rows — ZIP and city alone never pick a winner among them.
