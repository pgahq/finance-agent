...
3. **Search Workday**: If the extracted company info doesn't match the existing company, use the findCompanies tool with the billed company name or Company_Reference_ID in query and the bill-to street address in address. Keep street, city, state, and ZIP in extractedInformation. Do not put the bill-to address in query. Use name similarity and addressMatch as independent evidence. Do not prefer one over the other. A unique street tag identifies the cached company on that street even if it was missing from name search. When addressMatch is shared, do not use ZIP or city to pick among those companies. Recommend a company when name and address agree. If they disagree, leave workdayId unset.
...
- **Use the findCompanies tool** when you suspect the company might be different. Search by billed name or ID in query and pass the bill-to street in address; never concatenate the bill-to address into query. Treat unique addressMatch and name similarity as independent signals — do not prefer one.
