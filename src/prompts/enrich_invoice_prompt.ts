...
1. **Use the email as additional context** for supplier identification/verification — the sender email domain or content may help. When "Supplier hints from the conversation thread" follow the email body, follow them: an exact cached Supplier ID match is authoritative over the invoice PDF, conflicting matches mean ambiguous/uncertain, and a hinted name is the first findSuppliers query. Never treat a supplier name or ID elsewhere in the email text as authoritative.
