...
description: >-
  ...
  order and pay-file-safe characters), or Slack Workday object deep links.
---
...
8. Upload each vendor file to S3 (`new-invoices/{requestId}/{index}-{sanitizedFileName}`) and the transcript once (`new-invoices/{requestId}/{fileName}`) via `ingestCreateInvoiceAttachments`, then async-invoke `CreateInvoiceProcessor` with `emailContext` (`emailFrom` / `subject` from the attachment’s message when applicable; `plainTextBody` is the **source email body plus every non-empty `conversation_parts` body**, joined with blank lines in API order — including internal notes, not only the message that holds the PDF), `assigneeEmail`, `conversationCreatedAt` from Intercom `created_at`, and `conversationPdf` (`s3Key` / `fileName` of the transcript). When `INVOICE_ATTACHMENT_CLUSTERING_ENABLED` is `true`, the Intercom trigger passes a grouped invoke so the processor gets **one record per conversation** with `attachments` (every uploaded file's metadata, including `receivedAt`) and `latestMessageAt`; otherwise (default, including a missing SSM parameter), and always for the Gmail trigger, invoke **once per vendor PDF**. When it is `shadow`, the Intercom trigger invokes once per vendor PDF and then, best-effort, once more per conversation with `attachments` and `shadow: true` (report-only clustering Slack; never writes). Processor Lambda async retries are off (`MaximumRetryAttempts: 0`); a thrown error Slacks once and does not re-run.
9. Flag off: each vendor PDF creates a separate Workday invoice. Flag on: the processor classifies and clusters attachments (see the `invoice-attachment-clustering` skill) and creates **one Workday invoice per invoice cluster**; leftover clusters fan out to separate processor invokes. SOAP `Attachment_Data` lists the vendor PDF(s) first and the conversation transcript last. Enrichment sees vendor PDFs only, never the transcript. A re-triggered conversation never creates a second invoice for the same supplier invoice number: newer documents or messages update the existing Draft invoice, nothing-new re-triggers skip, and non-editable invoices skip with a manual-review note. Return HTTP status to the Data Connector
...
| `INVOICE_ATTACHMENT_CLUSTERING_ENABLED` | SSM `/finance-agent/invoice-attachment-clustering-enabled` (plain String, created by hand per account; not in the template or CircleCI) | Runtime toggle for create-invoice attachment clustering (one invoke per conversation, one Workday invoice per cluster). Only the exact value `true` enables it; `shadow` keeps one invoice per PDF and Slacks the clustering plan only; a missing parameter resolves to empty and leaves it off. `lambda-env` reads SSM once per container, so a flip applies as new containers start; update the function configuration to force it immediately. Read inside the handler after `loadEnv()`, never as a module-level constant. |
...
- Each Workday invoice receives its vendor PDF(s) as `Attachment_Data` (invoice first; a cluster's PDFs when clustering is on), with the conversation transcript appended last. Each `Submit_Supplier_Invoice` call sets the full attachment list (omitted files are dropped), so resend updates send every cluster PDF plus a fresh transcript
