...
description: >-
  ...
  from Intercom internal notes (emailContext.internalNotes, Supplier IDs and
  Supplier:/Vendor: lines).
---
...
**Supplier hints from internal notes:** `enrichInvoiceFromAttachments` scans only `emailContext.internalNotes` (Intercom `note` parts from any author, including workflow notes; `author.type` is not checked) for Workday Supplier IDs (`S-` plus 4–6 digits, any case) and `Supplier:` / `Vendor:` lines (colon, or a dash with spaces; embedded IDs are stripped from names). Never scan the external email body, subject, or Workday inbound-email bodies for hints — a sender could otherwise steer supplier assignment. IDs are resolved in code by exact match on cached `metadata.supplierId` (`resolveSupplierIdHints`). An exact match is injected as authoritative over the invoice PDF (including verify mode, where it can change an assigned supplier) only when every extracted ID resolved to that same supplier; if any other hinted ID is unresolved, the notes are called incomplete and the match is only a findSuppliers candidate. Two or more distinct matches tell the model to return ambiguous/uncertain. An ID missing from the cache is listed as not a match; a failed lookup asks for a findSuppliers result whose Supplier ID matches exactly. Hinted names only set the first findSuppliers query, and are dropped when the ID match is authoritative. `enrich_invoice` (Workday inbound email) has no `internalNotes`, so it gets no hints.
