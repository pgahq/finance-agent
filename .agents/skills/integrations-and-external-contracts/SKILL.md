...
description: >-
  ...
  Gmail labels, bearer auth tokens, attachment upload contracts, Workday
  invoice memos, line item descriptions, supplier invoice numbers (check-print
  order and pay-file-safe characters), Slack Workday object deep links, or
  supplier matching hints from the email context (subject, source email, and
  conversation parts; Supplier IDs and Supplier:/Vendor: lines).
---
...
6. Resolve **assignee email** from the same conversation GET: among `conversation_parts.conversation_parts`, use the **last** part with `part_type === "custom_action_started"` and take `author.email` (any `author.type`). Forward as `assigneeEmail` on each processor invoke.
7. Build a text transcript PDF of the same conversation. The file name is `pga_corp_accounts_payable_<YYYY>_<MM>_<DD>_<conversationId>.pdf`, using `created_at` in America/Chicago (current Central date if `created_at` is missing). The PDF includes the source email (subject with HTML stripped, then body) and `conversation_parts` whose `part_type` is `comment` or `note` and whose body is non-empty, in API order. Assignments, attribute updates, custom actions, and close events are omitted. Notes are labeled `Note`. The header is one title: `Conversation with {custom_attributes.Brand}` when Brand is set, otherwise the conversation `title`, otherwise `Conversation`. If rendering fails, return 500 and do not invoke processors.
8. Upload each vendor file to S3 (`new-invoices/{requestId}/{index}-{sanitizedFileName}`) and the transcript once (`new-invoices/{requestId}/{fileName}`) via `ingestCreateInvoiceAttachments`, then async-invoke `CreateInvoiceProcessor` with `emailContext` (`emailFrom` / `subject` from the attachment’s message when applicable; `plainTextBody` is the **source email body plus every non-empty `conversation_parts` body**, joined with blank lines in API order — including internal notes, not only the message that holds the PDF; `conversationParts` is the same minus the source email body), `assigneeEmail`, `conversationCreatedAt` from Intercom `created_at`, and `conversationPdf` (`s3Key` / `fileName` of the transcript). Every trigger (Intercom and Gmail) invokes **once per vendor PDF**. When `INVOICE_ATTACHMENT_CLUSTERING_ENABLED` is `shadow`, the Intercom trigger then sends one more best-effort invoke per conversation with `shadow: true` and `attachments` (every uploaded file's metadata, including Intercom `receivedAt`); the processor only classifies and clusters it and Slacks the plan as `create_invoice_shadow` (see the `invoice-attachment-clustering` skill). A failed shadow invoke is logged and never fails the request. Processor Lambda async retries are off (`MaximumRetryAttempts: 0`); a thrown error Slacks once and does not re-run.
9. Each vendor PDF creates a separate Workday invoice. SOAP `Attachment_Data` on that invoice lists the vendor PDF first and the conversation transcript second. Enrichment still sees only the vendor PDF. Return HTTP status to the Data Connector
10. Workday `Invoice_Received_Date` on each new supplier invoice is set from the Intercom conversation `created_at` (mailbox receipt), formatted as `YYYY-MM-DD` like other SOAP dates
...
**Supplier hints from the email context:** `enrichInvoiceFromAttachments` scans the whole email context — `emailContext.subject` and `plainTextBody` (for Intercom, the source email plus every non-empty conversation part, any `part_type` or author; for `/enrich-invoice`, the Workday inbound email) — for Workday Supplier IDs (`S-` plus 4–6 digits, any case) and `Supplier:` / `Vendor:` lines (colon, or a dash with spaces; embedded IDs are stripped from names). Everything scanned becomes a hint, but only IDs that also appear in `emailContext.conversationParts` (Intercom notes and replies) can be authoritative. An ID found only in the source email or the Workday inbound email is a findSuppliers candidate and never overrides the invoice PDF — a sender could otherwise pick a real cached supplier. `/enrich-invoice` has no `conversationParts`, so its hints never replace an assigned supplier. IDs are resolved in code by exact match on cached `metadata.supplierId` (`resolveSupplierIdHints`). An exact match is injected as authoritative over the invoice PDF only when every extracted ID resolved to that same supplier and its ID appears in a conversation part; if any other hinted ID is unresolved, the hints are called incomplete and the match is only a findSuppliers candidate. Two or more distinct matches tell the model to return ambiguous/uncertain. An ID missing from the cache is listed as not a match; a failed lookup asks for a findSuppliers result whose Supplier ID matches exactly. Hinted names only set the first findSuppliers query, and are dropped when the ID match is authoritative. 
...
**Closed or Pending Close POs:** `parsePurchaseOrder` reads header `Purchase_Order_Document_Status_Reference` into `documentStatus` (`Document_Status_ID` plus descriptor). `isPurchaseOrderClosedForInvoicing` is true when either value normalizes to Closed or Pending Close; a missing status reads as open. Workday rejects invoice lines that link to lines on those POs, so create and enrich still use the PO for company, memo PO number, and line coding (worktags, LOB, splits, spend category) but submit with `omitPurchaseOrderLineReference`, which drops every `Purchase_Order_Line_Reference` (including OCR passthrough lines on update). In-memory `purchaseOrderLineId` stays set. If a status was missing or unrecognized and Workday returns "PO that is Closed or Pending Close" while a submitted line carries the reference, submit retries once with the same omit and records the `omitted PO line reference (PO closed or pending close)` fallback. Other `Purchase_Order_Line_Reference` faults (duplicate, canceled, wrong PO or company, closed individual PO line) do not trigger the omit. Either path adds the note and Slack fallback line "PO-XXXXXX is Closed or Pending Close; invoice lines were coded from the PO but not linked to PO lines." Choosing a different open PO is out of scope.
...
| `INVOICE_ATTACHMENT_CLUSTERING_ENABLED` | SSM `/finance-agent/invoice-attachment-clustering-enabled` (plain String, created by hand per account; not in the template or CircleCI) | Create-invoice attachment clustering mode. Only the exact value `shadow` does anything: invoices stay one per PDF and the processor Slacks a report-only clustering plan. Any other value, including `true` or a missing parameter, is off. `lambda-env` reads SSM once per container, so a flip applies as new containers start; update the function configuration to force it immediately. Read inside the handler after `loadEnv()` via `invoiceAttachmentClusteringMode`, never as a module-level constant. |
