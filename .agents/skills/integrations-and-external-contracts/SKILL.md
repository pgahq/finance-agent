...
| `INVOICE_ATTACHMENT_CLUSTERING_ENABLED` | SSM `/finance-agent/invoice-attachment-clustering-enabled` (plain String, created by hand per account; not in the template or CircleCI) | Runtime toggle for create-invoice attachment clustering (one invoke per conversation, one Workday invoice per cluster). Only the exact value `true` enables it; a missing parameter resolves to empty and leaves it off. `lambda-env` reads SSM once per container, so a flip applies as new containers start; update the function configuration to force it immediately. Read inside the handler after `loadEnv()`, never as a module-level constant. |
