...
description: >-
  ...
  characters), Slack Workday object deep links, or supplier matching hints
  from the email context (subject, source email, and conversation parts;
  Supplier IDs and Supplier:/Vendor: lines).
---
...
7. Async-invoke `CreateInvoiceProcessor` once per vendor PDF with `emailContext` (`emailFrom` / `subject` from the attachment’s message when applicable; `plainTextBody` is the **source email body plus every non-empty `conversation_parts` body**, joined with blank lines in API order — including internal notes, not only the message that holds the PDF; `conversationParts` is the same minus the source email body), shared `assigneeEmail`, `conversationCreatedAt` from Intercom `created_at`, and `conversationPdf` (`s3Key` / `fileName` of the transcript). Processor Lambda async retries are off (`MaximumRetryAttempts: 0`); a thrown error Slacks once and does not re-run.
8. Each vendor PDF creates a separate Workday invoice. SOAP `Attachment_Data` on that invoice lists the vendor PDF first and the conversation transcript second. Enrichment still sees only the vendor PDF. Return HTTP status to the Data Connector
9. Workday `Invoice_Received_Date` on each new supplier invoice is set from the Intercom conversation `created_at` (mailbox receipt), formatted as `YYYY-MM-DD` like other SOAP dates
