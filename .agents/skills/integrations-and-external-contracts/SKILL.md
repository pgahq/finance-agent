...
Create-invoice Slack success and error details include `conversationId` and an inbox permalink from the stack `INTERCOM_APP_ID` (sandbox workspace `c722leqk`, prod `jyi16dpc`). Conversation `app_id` is only a fallback when that env is unset. Create-invoice Slack **errors** show the Workday `Message` plus prior submit attempts on the parent webhook message. The previous full error JSON (`name`, original `message`, `stack`, `priorFailures`, invoice `details`) is appended under *Full error* on that same incoming-webhook post — this stack has `SLACK_WEBHOOK_URL` only, so Slack threads are not available. Remaining invoice details (`fileName`, `s3Key`, `workdayId`, not-authorized `note`) still appear as JSON; `conversationUrl` stays a footer link. Create success JSON is truncated to Slack's context cap. Sanitized SOAP throws keep a `Validation_Fault` object so enrich skip-registry classification still matches, and keep `serializedError` for that *Full error* dump. Successful Workday submits that retried after a validation fault also include `priorFailures` on the Slack success payload (create JSON details and enrich `Prior submit failures` section).
