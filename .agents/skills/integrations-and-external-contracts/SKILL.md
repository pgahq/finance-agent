...
7. Async-invoke `CreateInvoiceProcessor` with `emailContext` (`emailFrom` / `subject` from the attachment’s message when applicable; `plainTextBody` is the **source email body plus every non-empty `conversation_parts` body**, joined with blank lines in API order — including internal notes, not only the message that holds the PDF), shared `assigneeEmail`, `conversationCreatedAt` from Intercom `created_at`, and `conversationPdf` (`s3Key` / `fileName` of the transcript). When `INVOICE_ATTACHMENT_CLUSTERING_ENABLED` is `true`, invoke **once per conversation** with `attachments` (every uploaded file's metadata); otherwise (default, including a missing SSM parameter), invoke **once per vendor PDF**. When it is `shadow`, invoke once per vendor PDF and then, best-effort, once more per conversation with `attachments` and `shadow: true` (report-only clustering Slack; never writes). Processor Lambda async retries are off (`MaximumRetryAttempts: 0`); a thrown error Slacks once and does not re-run.
8. Flag off: each vendor PDF creates a separate Workday invoice. Flag on: the processor classifies and clusters attachments (see the `invoice-attachment-clustering` skill) and creates **one Workday invoice per invoice cluster**; leftover clusters fan out to separate processor invokes. SOAP `Attachment_Data` lists the vendor PDF(s) first and the conversation transcript last. Enrichment sees vendor PDFs only, never the transcript. A re-triggered conversation never creates a second invoice for the same supplier invoice number: newer documents update the existing Draft invoice, nothing-new re-triggers skip, and non-editable invoices skip with a manual-review note. Return HTTP status to the Data Connector
